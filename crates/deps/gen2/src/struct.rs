...
pub fn gen_struct(def: &TypeDef, gen: &Gen) -> TokenStream {
    ...
    if gen.sys {
        ...
        gen_win_struct_with_name(def, def.name(), gen, &quote! {}, &quote! {})
    }
}
...
fn gen_win_struct_with_name(def: &TypeDef, struct_name: &str, gen: &Gen, arch_cfg: &TokenStream, feature_cfg: &TokenStream) -> TokenStream {
    ...
    let nested_types = gen_nested_win_structs(struct_name, def, gen, &arch_cfg, &feature_cfg);
...
fn gen_nested_win_structs<'a>(enclosing_name: &'a str, enclosing_type: &'a TypeDef, gen: &Gen, arch_cfg: &TokenStream, feature_cfg: &TokenStream) -> TokenStream {
    if let Some(nested_types) = enclosing_type.nested_types() {
        nested_types
            .iter()
            .enumerate()
            .map(|(index, (_, nested_type))| {
                let nested_name = format!("{}_{}", enclosing_name, index);
                gen_win_struct_with_name(nested_type, &nested_name, gen, &arch_cfg, &feature_cfg)
            })
            .collect()
    } else {
        TokenStream::new()
